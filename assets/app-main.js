...
function renderAI(){
  return`<section>
    ...
    <button class="button primary" id="aiGenerateBtn" onclick="generateAIPicks()" style="margin-bottom:16px">${state.aiLoading?'Generating…':'⚡ Generate '+(state.aiMode==='picks'?'Top Picks':state.aiMode==='stacks'?'Stack Ranks':'Sharp Edges')}</button>
    ...
    ${state.aiResult?`<div class="card" style="padding:22px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><div class="eyebrow">Claude Sonnet · ${escapeHtml(state.aiResultMode||'')} · ${escapeHtml(state.aiResultDate||'')}</div><button class="button" onclick="navigator.clipboard.writeText(document.getElementById('aiResultText').innerText).catch(()=>{})">Copy</button></div><div id="aiResultText" style="white-space:pre-wrap;font-size:14px;line-height:1.7;color:#c8d8e8">${escapeHtml(state.aiResult)}</div></div>`:!state.aiLoading?`<div class="card empty">Tap Generate to get Claude's analysis of today's slate.</div>`:`<div class="card loading"><strong>Claude is analyzing your slate…</strong><br><br>Sending ${state.games.length} games to Claude Sonnet.</div>`}
  </section>`;
}
