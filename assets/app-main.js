...
function renderStacks(){ const top = state.stackRows.slice(0,12); return ` <section id="stacksSection"> <div class="section-title"><h2>Stack Lab</h2><div class="meta">Your fast slate-prioritization lane</div></div> <div class="grid-3"> ${top.map(r=>`<div class="card callout"> <div class="row"><h3>${r.team} stack</h3><span class="tag ${r.style}">${r.level}</span></div> <p>Attack ${r.opponent} in ${escapeHtml(r.venue)}. Opposing starter is ${escapeHtml(r.oppPitcher)} and this offense lands a ${r.score}/99 stack score.</p> </div>`).join('') || '<div class="card empty">No stack board available.</div>'} </div> </section> `; }
function playerCards(players){ if(!players.length) return '<div class="empty">Could not load hitter pool for this team.</div>'; return players.map(p => ` <div class="player-card"> <div> <div class="player-name">${escapeHtml(p.name)}</div> <div class="player-meta">${escapeHtml(p.pos)} · ${escapeHtml(p.batSide||'R')} bat · ${fmtPct(p.avg)} AVG · ${fmtPct(p.ops)} OPS · ${p.hr} HR · ${p.rbi} RBI</div> <div class="mini-grid"> <div class="mini-chip">${escapeHtml(p.grade.splits.lrLabel)} · ${fmtPct(p.grade.splits.lrAvg)} AVG · ${fmtPct(p.grade.splits.lrOps)} OPS</div> <div class="mini-chip">${escapeHtml(p.grade.splits.venueLabel)} · ${fmtPct(p.grade.splits.venueObp)} OBP · ${fmtPct(p.grade.splits.venueSlg)} SLG</div> <div class="mini-chip">Market · ${p.grade.collab?.market?.score||'-'} · ${escapeHtml(p.grade.collab?.market?.label||'Neutral')}</div> <div class="mini-chip">Pattern · ${p.grade.collab?.pattern?.score||'-'} · ${escapeHtml(p.grade.collab?.pattern?.label||'Neutral')}</div> <div class="mini-chip">System · ${p.grade.collab?.system?.score||'-'} · ${escapeHtml(p.grade.collab?.system?.label||'Neutral')}</div> <div class="mini-chip">Bullpen · ${p.grade.collab?.pattern?.bullpen?.score||'-'} · ${escapeHtml(p.grade.collab?.pattern?.bullpen?.label||'Mixed')}</div> </div> <div class="reasons">${p.grade.reasons.map(r=>`<span>${escapeHtml(r)}</span>`).join(' · ')}</div> </div> <div> <div class="grade">${p.grade.letter}</div> <div class="grade-sub">${p.grade.score}/99</div> <button class="button watch-btn ${isWatched('hitter',p.name)?'active':''}" data-watch-hitter="${escapeHtml(p.name)}" style="margin-top:10px">${isWatched('hitter',p.name)?'Saved':'Watch'}</button> </div> </div> `).join(''); }
    ...
    <div class="grid-2">
      <div class="card callout">
        ...
        <div class="shell-form-grid">
          ...
          <label class="label full">Odds API Key (The Odds API)<input id="oddsApiKey" class="field" value="${escapeHtml(currentOddsKey)}" placeholder="Enter your odds API key" /></label>
...
function renderLaunchpad(){ const proxy=state.apiConfig?.proxyBaseUrl||'https://newest-mlb-1.onrender.com'; const oddsReady=state.liveSync.odds?.status==='ok'; const weatherReady=state.liveSync.weather?.status==='ok'; const selected=state.selectedGameData||state.games[0]||null; const checklist=[ {label:'Slate loads from MLB Stats API',ok:state.games.length>0,detail:state.games.length?`${state.games.length} games for ${state.selectedDate}`:'Pick a date and refresh'}, {label:'Private proxy URL saved',ok:/^https?:\/\//.test(proxy),detail:proxy}, {label:'Odds API key configured',ok:!!(state.apiConfig.oddsApiKey||'30b9f498731b8fa9f78a6aefd7764f3a'),detail:(state.apiConfig.oddsApiKey||'30b9f498731b8fa9f78a6aefd7764f3a')?'Key loaded':'Set in Odds + Weather tab'}, {label:'Odds sync passed',ok:oddsReady,detail:oddsReady?`Last sync ${fmtStamp(state.liveSync.odds.updatedAt)}`:'Run Sync Odds Now'}, {label:'Weather sync passed',ok:weatherReady,detail:weatherReady?`Last sync ${fmtStamp(state.liveSync.weather.updatedAt)}`:'Run Sync Weather Now'}, {label:'Selected matchup ready',ok:!!selected,detail:selected?`${selected.away.abbr} @ ${selected.home.abbr}`:'Open any game card'}, ]; return ` <section id="launchSection"> <div class="section-title"><h2>Launchpad</h2><div class="meta">Pre-first-pitch checklist</div></div> <div class="grid-2"> <div class="card callout"> <h3>Go-live checklist</h3> <div style="display:grid;gap:10px;margin-top:12px"> ${checklist.map(item=>`<div class="player-card" style="grid-template-columns:auto 1fr;align-items:center"><div class="grade" style="font-size:28px;text-align:left">${item.ok?'✓':'•'}</div><div><div class="player-name">${escapeHtml(item.label)}</div><div class="player-meta">${escapeHtml(item.detail)}</div></div></div>`).join('')} </div> <div class="hero-actions" style="margin-top:14px"> <button class="button primary" id="launchSyncAll">Run Full Sync</button> <button class="button" id="launchToMarket">Odds + Weather</button> <button class="button" id="launchToNotes">Notes</button> </div> </div> <div class="card callout"> <h3>Opening day flow</h3> <p>1) Refresh slate. 2) Confirm proxy URL. 3) Sync weather and odds. 4) Open best stack from Edge Board. 5) Validate hitter grades in Hitter Lab. 6) Save notes before lineup lock.</p> </div> </div> </section> `; }
...
function render(){
  ...
  if($('#saveApiConfigBtn')) $('#saveApiConfigBtn').onclick=()=>{
    saveApiConfig({
      proxyBaseUrl:$('#proxyBaseUrl')?.value.trim()||'https://newest-mlb-1.onrender.com',
      oddsApiKey:$('#oddsApiKey')?.value.trim()||'30b9f498731b8fa9f78a6aefd7764f3a',
      oddsRegion:$('#oddsRegion')?.value||'us',
      oddsBookmaker:$('#oddsBookmaker')?.value.trim()||'',
      autoSyncWeather:!!$('#autoSyncWeather')?.checked,
      autoSyncOdds:!!$('#autoSyncOdds')?.checked
    });
    render();
  };
