function renderEdges(){
  return `
    <section>
      <div class="section-title"><h2>Edge Board</h2><div class="meta">Exportable offense ranking · ${filteredStackRows().length} rows shown</div></div>
      <div class="card table-wrap">
        <table>
          <thead>
            <tr><th>Rank</th><th>Team</th><th>Side</th><th>Opponent</th><th>Score</th><th>Level</th><th>Watch</th><th>Opp Pitcher</th><th>Venue</th></tr>
          </thead>
          <tbody>
            ${filteredStackRows().map((r,i)=>`<tr>
              <td class="mono">${i+1}</td>
              <td><strong>${r.team}</strong></td>
              <td>${r.side}</td>
              <td>${r.opponent}</td>
              <td class="mono">${r.score}/99</td>
              <td><span class="tag ${r.style}">${r.level}</span></td>
              <td><button class="button watch-btn ${isWatched('team',r.team)?'active':''}" data-watch-team="${r.team}" data-watch-game="${r.gamePk}">${isWatched('team',r.team)?'Saved':'Watch'}</button></td>
              <td>${escapeHtml(r.oppPitcher)}</td>
              <td>${escapeHtml(r.venue)}</td>
            </tr>`).join('') || '<tr><td colspan="9" class="empty">No edges available.</td></tr>'}
          </tbody>
        </table>
      </div>
    </section>
...
function renderStacks(){
  ...
  return `
    <section id="stacksSection">
      <div class="section-title"><h2>Stack Lab</h2><div class="meta">Your fast slate-prioritization lane</div></div>
      <div class="grid-3">
        ${top.map(r=>`<div class="card callout">
          <div class="row"><h3>${r.team} stack</h3><span class="tag ${r.style}">${r.level}</span></div>
          <p>Attack ${r.opponent} in ${escapeHtml(r.venue)}. Opposing starter is ${escapeHtml(r.oppPitcher)} and this offense lands a ${r.score}/99 stack score.</p>
        </div>`).join('') || '<div class="card empty">No stack board available.</div>'}
      </div>
    </section>
  `;
}
...
function renderHitterLab(){
  ...
  return `
    <section>
      <div class="section-title"><h2>Hitter Lab</h2><div class="meta">Matchup grading with splits, travel, starter tendencies, bullpen projection</div></div>
      <div class="card" style="padding:16px;margin-bottom:16px">
        <div class="row" style="align-items:flex-start;gap:18px;flex-wrap:wrap">
          <div>
            <div class="eyebrow">Selected Game</div>
            <div class="hero-title" style="font-size:42px;margin:8px 0 4px">${g.away.abbr} @ ${g.home.abbr}</div>
            <div class="mini">${escapeHtml(g.venue.name)} · ${fmtTime(g.gameDate)} · Run ${fmtNum(g.park.run,2)} · HR ${fmtNum(g.park.hr,2)}</div>
          </div>
          <div class="lift" style="min-width:340px;flex:1">
            <div class="stat"><div class="k">Away starter</div><div class="v">${fmtNum(g.awayPitcher.era,2)}</div><div class="s">${escapeHtml(g.awayPitcher.name)} ERA</div></div>
            <div class="stat"><div class="k">Home starter</div><div class="v">${fmtNum(g.homePitcher.era,2)}</div><div class="s">${escapeHtml(g.homePitcher.name)} ERA</div></div>
            <div class="stat"><div class="k">Best bat</div><div class="v">${Math.max(g.awayHitters[0]?.grade.score||0,g.homeHitters[0]?.grade.score||0)}</div><div class="s">Top hitter grade</div></div>
          </div>
        </div>
        <div class="insight-grid">
          ${renderTravelPanel(g.away.abbr,g.awayTravel)}
          ${renderTravelPanel(g.home.abbr,g.homeTravel)}
          ${renderPitcherPanel(g.away.abbr,g.awayPitcher,g.awayStarterTendencies)}
          ${renderPitcherPanel(g.home.abbr,g.homePitcher,g.homeStarterTendencies)}
          ${renderBullpenPanel(g.away.abbr,g.awayBullpen)}
          ${renderBullpenPanel(g.home.abbr,g.homeBullpen)}
        </div>
      </div>
      <div class="split">
        <div class="card team-panel">
          <div class="panel-title"><h3>${g.away.abbr} hitters</h3><span class="tag ${stackLevel(teamEdgeScore(g,'away'))[1]}">${stackLevel(teamEdgeScore(g,'away'))[0]}</span></div>
          <div class="mini" style="margin-bottom:12px">Facing ${escapeHtml(g.homePitcher.name)} · ${escapeHtml(g.homeStarterTendencies.profile)}</div>
          <div class="trend-list" style="margin-bottom:12px">
            ${bullpenRoleBlock('Closer',g.homeBullpen?.closer?[g.homeBullpen.closer]:[])}
            ${bullpenRoleBlock('Setup',g.homeBullpen?.setup||[])}
          </div>
          ${playerCards(g.awayHitters)}
        </div>
        <div class="card team-panel">
          <div class="panel-title"><h3>${g.home.abbr} hitters</h3><span class="tag ${stackLevel(teamEdgeScore(g,'home'))[1]}">${stackLevel(teamEdgeScore(g,'home'))[0]}</span></div>
          <div class="mini" style="margin-bottom:12px">Facing ${escapeHtml(g.awayPitcher.name)} · ${escapeHtml(g.awayStarterTendencies.profile)}</div>
          <div class="trend-list" style="margin-bottom:12px">
            ${bullpenRoleBlock('Closer',g.awayBullpen?.closer?[g.awayBullpen.closer]:[])}
            ${bullpenRoleBlock('Setup',g.awayBullpen?.setup||[])}
          </div>
          ${playerCards(g.homeHitters)}
        </div>
      </div>
    </section>
  `;
...
function renderSignals(){
  ...
  const watchHitters=state.watchlist.filter(x=>x.type==='hitter').slice(0,8);
  const bestParks=state.games.map(g=>({matchup:`${g.away.abbr} @ ${g.home.abbr}`,venue:g.venue.name,run:parkFor(g.venue.name).run,hr:parkFor(g.venue.name).hr})).sort((a,b)=>b.run-a.run).slice(0,6);
  return `
    <section>
      <div class="section-title"><h2>Signals</h2><div class="meta">Fast-glance slate pressure board</div></div>
      <div class="signal-grid">
        <div class="card signal-card"><h3>Attackable pitchers</h3><div class="mini">${attackable.map(p=>`${p.name} → ${p.opp} (${p.weak})`).join('<br>')||'No slate loaded.'}</div></div>
        <div class="card signal-card"><h3>Best parks</h3><div class="mini">${bestParks.map(p=>`${p.matchup} · ${p.venue} · run ${fmtNum(p.run,2)}`).join('<br>')||'No parks.'}</div></div>
        <div class="card signal-card"><h3>Watched teams</h3><div class="mini">${watchTeams.length?watchTeams.map(t=>t.name).join('<br>'):'No saved stacks yet.'}</div></div>
        <div class="card signal-card"><h3>Watched hitters</h3><div class="mini">${watchHitters.length?watchHitters.map(t=>t.name).join('<br>'):'No saved hitters yet.'}</div></div>
      </div>
      <div class="grid-2" style="margin-top:16px">
        <div class="card callout"><h3>Signals board</h3><p>Your quick decision layer: weakest starters, best run environments, and your saved stacks and hitters.</p></div>
        <div class="card callout" style="cursor:pointer" onclick="state.tab='ai';render()"><h3>⚡ AI Picks →</h3><p>Tap to open Claude AI analysis — GPP stacks, sharp edges, and pitcher fades.</p></div>
      </div>
    </section>
  `;
}
...
function renderAI(){
  return `
    <section>
      <div class="section-title"><h2>⚡ AI Picks</h2><div class="meta">Claude Sonnet analysis powered by your slate data</div></div>
      <div class="card callout" style="margin-bottom:16px"><h3>How it works</h3><p>Your live slate — pitchers, parks, matchups — is sent to Claude. Pick a mode and tap Generate. Takes about 5 seconds.</p></div>
      ...
      <button class="button primary" id="aiGenerateBtn" onclick="generateAIPicks()" style="margin-bottom:16px">
        ${state.aiLoading?'Generating…':'⚡ Generate '+(state.aiMode==='picks'?'Top Picks':state.aiMode==='stacks'?'Stack Ranks':'Sharp Edges')}
      </button>
      ...
      ${state.aiResult?`
        <div class="card" style="padding:22px">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
            <div class="eyebrow">Claude Sonnet · ${escapeHtml(state.aiResultMode||'')} · ${escapeHtml(state.aiResultDate||'')}</div>
            <button class="button" onclick="navigator.clipboard.writeText(document.getElementById('aiResultText').innerText).catch(()=>{})">Copy</button>
          </div>
          <div id="aiResultText" style="white-space:pre-wrap;font-size:14px;line-height:1.7;color:#c8d8e8">${escapeHtml(state.aiResult)}</div>
        </div>
      `:!state.aiLoading?`<div class="card empty">Tap Generate to get Claude's analysis of today's slate.</div>`:`<div class="card loading"><strong>Claude is analyzing your slate…</strong><br><br>Sending ${state.games.length} games to Claude Sonnet.</div>`}
    </section>
  `;
}
...
function renderMarket(){
  ...
  return `
    <section>
      <div class="section-title"><h2>Live Odds + Weather</h2><div class="meta">Sync odds and weather through your private backend</div></div>
      <div class="grid-2">
        <div class="card callout">
          ...
          <div class="shell-form-grid">
            ...
            <label class="label"><span class="tiny">Auto odds</span><br><input id="autoSyncOdds" type="checkbox" ${state.apiConfig.autoSyncOdds?'checked':''}/> odds proxy</label>
          </div>
      ...
      <div class="shell-grid" style="margin-top:16px">
        ...
        <div style="display:grid;gap:16px">
          ...
          <div class="card signal-card">
            <h3>All games</h3>
            <div class="mini">${all.map(x=>`${x.g.away.abbr}@${x.g.home.abbr} · ${x.m.total?`O/U ${x.m.total}`:'no total'} · ${x.wText}`).join('<br>')||'No games.'}</div>
          </div>
        </div>
      </div>
    </section>
...
function renderLaunchpad(){
  ...
  const checklist=[
    {label:'Slate loads from MLB Stats API',ok:state.games.length>0,detail:state.games.length?`${state.games.length} games for ${state.selectedDate}`:'Pick a date and refresh'},
    {label:'Private proxy URL saved',ok:/^https?:\/\//.test(proxy),detail:proxy},
    {label:'Odds sync passed',ok:oddsReady,detail:oddsReady?`Last sync ${fmtStamp(state.liveSync.odds.updatedAt)}`:'Run Sync Odds Now'},
    {label:'Weather sync passed',ok:weatherReady,detail:weatherReady?`Last sync ${fmtStamp(state.liveSync.weather.updatedAt)}`:'Run Sync Weather Now'},
    {label:'Selected matchup ready',ok:!!selected,detail:selected?`${selected.away.abbr} @ ${selected.home.abbr}`:'Open any game card'},
  ];
  return `
    <section id="launchSection">
      <div class="section-title"><h2>Launchpad</h2><div class="meta">Pre-first-pitch checklist</div></div>
      <div class="grid-2">
        <div class="card callout">
          <h3>Go-live checklist</h3>
          <div style="display:grid;gap:10px;margin-top:12px">
            ${checklist.map(item=>`<div class="player-card" style="grid-template-columns:auto 1fr;align-items:center"><div class="grade" style="font-size:28px;text-align:left">${item.ok?'✓':'•'}</div><div><div class="player-name">${escapeHtml(item.label)}</div><div class="player-meta">${escapeHtml(item.detail)}</div></div></div>`).join('')}
          </div>
          <div class="hero-actions" style="margin-top:14px">
            <button class="button primary" id="launchSyncAll">Run Full Sync</button>
            <button class="button" id="launchToMarket">Odds + Weather</button>
            <button class="button" id="launchToNotes">Notes</button>
          </div>
        </div>
        <div class="card callout">
          <h3>Opening day flow</h3>
          <p>1) Refresh slate. 2) Confirm proxy URL. 3) Sync weather and odds. 4) Open best stack from Edge Board. 5) Validate hitter grades in Hitter Lab. 6) Save notes before lineup lock.</p>
        </div>
      </div>
    </section>
  `;
...
function renderPricing(){
  return `
    <section id="pricingSection">
      ...
      <div class="pricing">
        <div class="card price-card"><h3>Starter</h3><div class="price">$19<span style="font-size:18px">/mo</span></div><ul class="clean"><li>✓ Dashboard</li><li>✓ Edge board</li><li>✓ Stack lab</li><li>✓ Notes</li></ul><button class="button primary" data-plan-checkout="starter" style="width:100%;margin-top:12px">Start Starter</button></div>
        <div class="card price-card featured"><h3>Pro</h3><div class="price">$49<span style="font-size:18px">/mo</span></div><ul class="clean"><li>✓ Hitter lab</li><li>✓ Live odds + weather</li><li>✓ ⚡ Claude AI picks</li><li>✓ CSV export</li></ul><button class="button primary" data-plan-checkout="pro" style="width:100%;margin-top:12px">Start Pro</button></div>
        <div class="card price-card"><h3>Elite</h3><div class="price">$99<span style="font-size:18px">/mo</span></div><ul class="clean"><li>✓ Everything in Pro</li><li>✓ Priority support</li><li>✓ Future releases</li></ul><button class="button primary" data-plan-checkout="elite" style="width:100%;margin-top:12px">Start Elite</button></div>
      </div>
    </section>
  `;
}
  ...
  return `
    <section>
      ...
      <div class="card notes">
        <textarea id="notesBox" placeholder="Attack CIN in GABP if wind out. Avoid low-total parks unless starter cracked.">${escapeHtml(state.notes)}</textarea>
        <div class="hero-actions" style="margin-top:12px">
          <button id="saveNotes" class="button primary">Save Notes</button>
          <button id="clearNotes" class="button">Clear</button>
        </div>
      </div>
    </section>
  `;
...
function render(){
  ...
  if(!state.games.length&&state.tab!=='pricing'&&state.tab!=='notes'&&state.tab!=='launch'){ view.innerHTML=`<div class="card empty">No games loaded for ${escapeHtml(state.selectedDate)}.</div>`; return; }
...
(function(){
  ...
  if(params.get('checkout')==='success'){ history.replaceState(null,'',window.location.pathname); openGate(); msg('Payment confirmed! Enter your checkout email and click "Just Paid? Claim Token".','ok'); }
    ...
    (async function(){ const base=getBase(); try{ const res=await fetch(base+'/api/auth/verify',{headers:{'Authorization':'Bearer '+_savedToken}}); const data=await res.json().catch(()=>({})); if(res.ok&&data.ok){ closeGate(data.plan,data.email,base); }else{ localStorage.removeItem('allday-mlb-edge-token'); openGate(data.error||'Session expired.','err'); } }catch(e){ openGate('Could not reach backend. Check the Backend URL above.','err'); } })();
  } else { openGate(); }
})();
