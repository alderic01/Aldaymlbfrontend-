...
async function generateAIPicks(){
  ...
  try{
    ...
    const resp=await fetch(_aiBase+'/api/ai-picks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({games,date:state.selectedDate,mode:state.aiMode,dkSalaries:(()=>{const all=Object.values(state.dkSalaries||{});const byPos={};all.forEach(p=>{const pg=p.pos&&p.pos.includes('SP')?'SP':p.pos&&p.pos.includes('C')&&!p.pos.includes('CF')?'C':p.pos&&/1B/.test(p.pos)?'1B':p.pos&&/2B/.test(p.pos)?'2B':p.pos&&/3B/.test(p.pos)?'3B':p.pos&&/SS/.test(p.pos)?'SS':p.pos&&/OF/.test(p.pos)?'OF':'OTHER';if(pg!=='OTHER'&&pg!=='RP'||p.pos==='RP'){if(!byPos[p.pos])byPos[p.pos]=[];byPos[p.pos].push(p);}});const out=[];['SP','C','1B','2B','3B','SS','OF'].forEach(g=>{const grp=all.filter(p=>p.pos&&(g==='SP'?p.pos==='SP':g==='OF'?/OF/.test(p.pos):new RegExp(g).test(p.pos))).sort((a,b)=>(b.avgPts||0)-(a.avgPts||0));grp.slice(0,g==='SP'?8:g==='OF'?8:6).forEach(p=>out.push({n:p.name,pos:g,sal:p.salary,pts:p.avgPts,team:p.team}));});return out;})()})});
