...
def health():
    return jsonify({
        ...
        "slate_ready":  bool(slatecache and "error" not in slatecache),  # FIX 1
    })
...
def get_hitters():
    ...
    if team:      hitters = [h for h in hitters if h.get("team") == team.upper()]
    if min_fires: hitters = [h for h in hitters if (h.get("fires") or 0) >= min_fires]
    if min_grade:
        min_idx = grade_order.get(min_grade, 5)
        hitters = [h for h in hitters if grade_order.get(h.get("grade", "D"), 5) <= min_idx]
    if min_sal:   hitters = [h for h in hitters if (h.get("dk_salary") or 0) >= min_sal]
    if max_sal:   hitters = [h for h in hitters if (h.get("dk_salary") or 99999) <= max_sal]
    ...
    return jsonify({
        "hitters":      [formathitter(h) for h in hitters],  # FIX 2
        "count":        len(hitters),
        "refreshed_at": slate.get("refreshed_at"),
    })
...
def get_pitchers():
    ...
    return jsonify({
        "pitchers":     [formatpitcher(p) for p in pitchers],  # FIX 2
        "count":        len(pitchers),
        "refreshed_at": slate.get("refreshed_at"),
    })
...
def get_player(player_id: int):
    ...
    for h in slate.get("hitters", []):
        if h.get("player_id") == player_id:
            return jsonify({"type": "hitter", "data": formathitter(h)})
    for p in slate.get("pitchers", []):
        if p.get("player_id") == player_id:
            return jsonify({"type": "pitcher", "data": formatpitcher(p)})
...
def get_salaries():
    return jsonify({
        "salaries": dksalaries,
        "count":    len(dksalaries),          # FIX 1: was _dk_salaries
...
def upload_salaries():
    ...
    for i, row in enumerate(reader):
        try:
            ...
            if name: parsed[name]    = salary
            if pid:  parsed[str(pid)] = salary
        except Exception as e:
            errors.append(f"Row {i}: {e}")

    with refreshlock:                         # FIX 7: thread-safe salary update
        dksalaries.update(parsed)

    refreshslate(force=True)                  # FIX 6: actually re-optimize after upload

    return jsonify({
        "loaded": len(parsed),
        "total":  len(dksalaries),            # FIX 1: was _dk_salaries
        "errors": errors[:10],
    })
...
@require_admin
def force_refresh():
    result = refreshslate(force=True)
    return jsonify({
        "status":       "refreshed",
...
def backgroundrefresh_loop():                 # FIX 3: was _background_refresh_loop in _startup
    log.info("Background refresh loop started (30-min interval)")
    while True:
        try:
            if isstale(max_age_minutes=30):
                refreshslate()
        except Exception as e:
...
def _startup():
    ...
    threading.Thread(target=backgroundrefresh_loop, daemon=True).start()  # FIX 3


if __name__ == "__main__":                    # FIX 4: was `if name ==`
    log.info("╔══════════════════════════════════════╗")
